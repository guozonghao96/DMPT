...
def get_num_layer_for_mae(var_name, num_encoder_layers, num_decoder_layers, num_max_layer):
    if var_name in ("backbone.cls_token", "backbone.mask_token", "backbone.pos_embed"):
        ...
        var_name.startswith("roi_head.bbox_head.norm") or 
        var_name.startswith("roi_head.bbox_head.decoder_embed") or
        var_name.startswith("roi_head.mask_head.norm") or 
        var_name.startswith("roi_head.mask_head.decoder_embed") or
        False
    ):
        return num_encoder_layers + 1
    elif (
        var_name.startswith("roi_head.bbox_head.decoder_blocks") or
        var_name.startswith("roi_head.mask_head.decoder_blocks") or
        False
    ):
...
class MAELayerDecayOptimizerConstructor(DefaultOptimizerConstructor):
    ...
    def add_params(self, params, module, prefix='', is_dcn_module=None):
        ...
        if isinstance(self.paramwise_cfg, (list, tuple, set)):
            assert len(self.paramwise_cfg) == 3
            num_encoder_layers, num_decoder_layers, layer_decay_rate = self.paramwise_cfg
        else:
        ...
        print("Build LayerDecayOptimizerConstructor %f - %d" % (layer_decay_rate, num_max_layer))
        ...
        for name, param in module.named_parameters():
            ...
            layer_id = get_num_layer_for_mae(name, num_encoder_layers, num_decoder_layers, num_max_layer)
