...
#     ),
    ...
    rpn_head=dict(
        # _delete_=True,
        type='ScaleCascadeRPNHead',
        num_stages=2,
        stages=[
            dict(
                type='ScaleStageCascadeRPNHead',
                in_channels=256,
                feat_channels=256,
                anchor_generator=dict(
                    type='AnchorGenerator',
                ...
                adapt_cfg=dict(type='dilation', dilation=3), # 这个参数没用
...
#                               use_sigmoid=False, # gfocal_loss 
                ...
                loss_bbox=dict(type='SmoothL1Loss', beta=0.11, loss_weight=0.075)),
                # loss_bbox=dict(type='IoULoss', linear=True, loss_weight=10.0)),
                # loss_bbox=dict(type='IoULoss', linear=True, loss_weight=1.0)),
            dict(
                type='ScaleStageCascadeRPNHead',
                in_channels=256,
                feat_channels=256,
                adapt_cfg=dict(type='offset'),
#                 adapt_cfg=dict(type='dilation', dilation=1),
                bridged_feature=False,
                sampling=True,
                with_cls=True,
                with_mil=False,
    ...
    train_cfg=dict(
        ...
        rpn=[
            dict(
                ...
                allowed_border=-1,
                pos_weight=-1,
                debug=False), # 实际并不用，而是用freeanchor的tokp进行
            dict(
                assigner=dict(
                    type='MaxIoUAssigner',
                    pos_iou_thr=0.7,
