...
model = dict(
    ...
    decoder_head=dict(
        type='PanopticDeepLabDecoderSame',
        ...
        instance_head=dict(
            ...
            decoder_channels=128,
            head_channels=(128, ) * num_scale,
            num_classes=(21, ) * num_scale,
            class_key=["seg{}".format(i + 1) for i in range(num_scale)]),
        loss_pred=dict(type='DeepLabCE', top_k_percent_pixels=1.0, loss_weight=1.0),
    ),
    offset_head=dict(
        type='PanopticDeepLabDecoderSame',
        instance_decoder=dict(
        ...
        instance_head=dict(
            type="SinglePanopticDeepLabHeadSame",
            decoder_channels=128,
            head_channels=(32, ) * num_scale,
            num_classes=(2, ) * num_scale,
            class_key=["offset{}".format(i + 1) for i in range(num_scale)]),
        loss_pred=dict(type='L1Loss', loss_weight=0.01),
    ),
    snake_head=None,
    # snake_head=dict(
    #     type='DeepSnakeHead',
    ...
    #         featmap_strides=[16]),
    #     decoder_head=dict(
    #         type='SnakeDecoderHead',
    #         state_dim=512, # 128 
    #         feature_dim=256,  # 64 = 32 * 2
    #         conv_type='dgrid',
    #         snake_config=dict(ro=16, adj_num=4),     
    #         loss_energy=dict(
