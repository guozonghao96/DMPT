...
#                                                                     vit_feat[i_img],
        ...
        for i_img in range(num_imgs):
            ...
            with torch.no_grad():
                ...
                add_instance_scores = (instance_root_score * root_mask.unsqueeze(-1)).flatten(1, 2).sum(1).unsqueeze(1) / (root_mask.flatten(1, 2).sum(1).unsqueeze(-1).unsqueeze(1) + 1) # num_gt, 1, 20
                ...
                part_labels = labels.reshape(-1, 1, 1).repeat(1, 
                                                              self.num_matchings ** 2,
                                                              self.num_semantic_points)
                # num_gt, num_matchings ^ 2, num_parts
                part_matching_losses = self.bce_loss(part_matching_scores.reshape(-1, self.num_classes),
                                                     part_labels.reshape(-1)).reshape(num_gt, 
                                                                                      self.num_matchings ** 2,
                                                                                      self.num_semantic_points,
                                                                                      self.num_classes).mean(-1)
                
                # 因为invisible的分数，计算出来是inf，我们需要用visible的权重让其变为0
                part_matching_losses = part_matching_losses * part_visibles[..., 0]
                # d_ = part_matching_losses.clone()
                # num_gt, 1, 1
                root_matching_loss = self.bce_loss(add_instance_scores.reshape(-1, self.num_classes).softmax(-1),
                                                     labels.reshape(-1)).reshape(num_gt, 1, 
                                                                                 self.num_classes).mean(-1)
                # (num_gt, num_matchings ^ 2, num_parts + num_gt, 1, 1).sum(-1) / (num_gt, num_matchings ^ 2, num_parts)
                # num_gt, num_matchings ^ 2 
                # bce losses 取平均用于选择，其实不取也行，同一个gt里面 invisible的分数是固定的
                matching_bce_losses = (part_matching_losses.sum(-1) + root_matching_loss) / \
                                      (part_visibles.squeeze(-1).sum(-1) + 1)
                # print(matching_bce_losses.shape)
                # 取loss最小的 num_gt(每个最小的匹配index)
                matching_index = matching_bce_losses.min(1)[1]
                # print(d_[torch.arange(num_gt).to(d_.device), matching_index])
                # print(matching_bce_losses.min(1)[0])
            ...
            for row_inds_, col_inds_, part_score_, deform_cost_, part_mask_, label_, part_centers_ in \
                ...
                # 直接logits.softmax * mask 来获取map，导致空间上deformable的响应被归一化，无法准确表示空间远近
                ...
                # 直接logits * mask 来获取map，可能mask=0的区域是响应的一个最大值，导致选择上的问题
                # masked_part_proposals_map.append(
                #     F.interpolate(((part_score_ - (part_deform_ * deform_cost_).sum(-1).unsqueeze(-1))
                #                    * part_mask_).permute(0, 3, 1, 2), 
                #                   size=(img_h, img_w),
                #                   mode='bilinear')[:, label_, ...] # num_parts, H, W
                # )
            ...
            # num_gt, (num_classifier + 1), num_classes -> num_gt, num_classes
            root_losses = self.bce_loss(masked_instance_root_score.softmax(-1).reshape(-1, self.num_classes), 
                                        labels.reshape(-1)).reshape(-1, self.num_classes).mean(-1) # num_gt, num_classes
            # num_gt, num_parts, num_classes
            part_losses = self.bce_loss(masked_instance_part_score.softmax(-1).reshape(-1, self.num_classes), 
                                        labels.reshape(-1, 1).repeat(1, self.num_semantic_points).reshape(-1))
            part_losses = part_visibles * part_losses.reshape(num_gt, self.num_semantic_points, self.num_classes).mean(-1)
            # print(part_losses)
            all_bce_losses = (root_losses + part_losses.sum(-1)) / (part_visibles.sum(-1) + 1)
            # print(all_bce_losses - matching_bce_losses.min(1)[0])
            # print(part_deform)
            # print()
            # object_score = torch.cat([masked_instance_root_score.softmax(-1), 
            #                           masked_instance_part_score.softmax(-1) * part_visibles.unsqueeze(-1)], dim=1).sum(1)
            # object_score = object_score / (part_visibles.sum(-1).unsqueeze(-1) + 1)
            losses_dpm['dpm_bce_loss'] += all_bce_losses.mean() / num_imgs + instance_part_score.sum() * 0
