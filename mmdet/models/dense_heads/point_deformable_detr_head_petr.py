...
class PointDeformableDETRHead(PointDETRHead):
    ...
    def _init_layers(self):
        ...
        reg_branch.append(Linear(self.embed_dims, 512))
        reg_branch.append(nn.ReLU())
        for _ in range(self.num_reg_fcs):
            reg_branch.append(Linear(512, 512))
            reg_branch.append(nn.ReLU())
        reg_branch.append(Linear(512, 2 * self.num_keypoints))
    ...
    def forward_train(self,
        ...
        for i_img in range(num_imgs):
            ...
            sampled_points, pos_point_feats, \
                visible_weights, semantic_points = self.get_semantic_centers(map_cos_fg[-1].clone(), 
                                                                    map_cos_bg[-1].clone(), 
                                                                    gt_bboxes[i_img], 
                                                                    vit_feat[i_img], 
                                                                    pos_thr=0.35,
...
#             query_embeds = self.query_embedding.weight
        ...
        hs, init_reference, inter_references, \
            enc_outputs_class, enc_outputs_coord, intermediate_shifted_points_init, memory = self.transformer(
                    mlvl_feats,
                    mlvl_masks,
                    query_embeds,
                    mlvl_positional_encodings,
        ...
        num_stage = hs.size(0)
        # 变换为框
        points_across_multattn = intermediate_shifted_points_init.reshape(num_stage,
                                                     num_imgs,
                                                     self.num_query,
                                                     -1, self.num_keypoints, 2)
        
        # transfered_bboxes是cxcywh的格式并且做到归一化的(demsatten 本身传的就是norm后的坐标)
        transfered_bboxes_xyxy = self.points2bbox(points_across_multattn) # num_stage, num_imgs, self.num_query, 8, 4
        transfered_bboxes_temp = transfered_bboxes_xyxy[..., 0, :] # 随便找一个头的bbox来匹配（实际上也没匹配，就是规范输入）
        transfered_bboxes_temp = bbox_cxcywh_to_xyxy(
            transfered_bboxes_temp.reshape(-1, 4)).reshape(num_stage, 
                                                           num_imgs, 
                                                           self.num_query, 
                                                           4)
        # transfered_bboxes是cxcywh的格式并且做到归一化的(demsatten 本身传的就是norm后的坐标)
        pred_transfered_bboxes = bbox_cxcywh_to_xyxy(
            transfered_bboxes_xyxy.reshape(-1, 4)).reshape(num_stage, 
                                                           num_imgs, 
                                                           self.num_query, 
                                                           -1, 4)
        ...
        if self.as_two_stage:
            outs = (outputs_classes, transfered_bboxes_temp, pred_transfered_bboxes, \
        ...
        #     fg_points_ = fg_points_.reshape(num_gt, -1)
        ...
        for fg_points_, labels_, weights_ in zip(all_semantic_points, gt_labels, all_visible_weights):
            ...
            fg_points_ = fg_points_.reshape(num_gt, -1)
            labels_ = labels_.reshape(-1)
            weights_ = weights_.unsqueeze(-1).expand(-1, -1, 2).reshape(num_gt, -1)
            gt_fg_points.append(fg_points_)
            gt_point_labels.append(labels_)
            gt_visible_weights.append(weights_)
            
        loss_inputs = outs + (gt_bboxes, gt_point_labels, gt_fg_points, gt_visible_weights, img_metas)
        losses, refine_targets = self.loss(*loss_inputs, gt_bboxes_ignore=None)
        
        # 准备part base dedetr
        # 先准备targets 和 reference points 和 gt labels
        pred_keypoints = []
        target_keypoints = []
        target_labels = []
        target_bboxes = []
        ...
        for pos_inds_, preds_, targets_, labels_, bbox_targets_, \
                row_inds_, col_inds_ in refine_targets:
            preds_ = torch.cat(preds_, dim=0)
            targets_ = torch.cat(targets_, dim=0)
            labels_ = torch.cat(labels_, dim=0)
            pred_keypoints.append(preds_[pos_inds_])
            target_keypoints.append(targets_[pos_inds_])
            target_labels.append(labels_[pos_inds_])
            
            bbox_targets_ = torch.cat(bbox_targets_, dim=0)
            target_bboxes.append(bbox_targets_[pos_inds_])
            # target_bboxes.append(bbox_targets_.reshape(-1, 8, 4)[pos_inds_])
            
            row_inds_ = torch.cat(row_inds_, dim=0)
            col_inds_ = torch.cat(col_inds_, dim=0)
            vis_row_inds.append(row_inds_)
            vis_col_inds.append(col_inds_)
            vis_pos_inds.append(pos_inds_)
        
        # for vis
        vis_pred_keypoints = torch.stack(pred_keypoints, dim=0)
        vis_target_keypoints = torch.stack(target_keypoints, dim=0)
        vis_pos_inds = torch.stack(vis_pos_inds, dim=0)
        vis_target_bboxes = torch.stack(target_bboxes, dim=0)
        # for vis
        
        pred_keypoints = torch.stack(pred_keypoints, dim=0)
        target_keypoints = torch.stack(target_keypoints, dim=0)
        target_labels = torch.stack(target_labels, dim=0)
        
        vis_row_inds = torch.stack(vis_row_inds, dim=0)
        vis_col_inds = torch.stack(vis_col_inds, dim=0)
        
        # reference points 一定要detach
        ref_poitns = pred_keypoints[-1].reshape(-1, self.num_keypoints, 2).detach()
        target_points = target_keypoints[-1].reshape(-1, self.num_keypoints, 2) # num_gt, num_keypoints, 2 已经被reshape到[0，1]
        target_labels = target_labels[-1]
        target_vis_row_inds = vis_row_inds[-1]
        target_vis_col_inds = vis_col_inds[-1]

        hs, init_reference, inter_references, \
            enc_outputs_class, enc_outputs_coord, \
                intermediate_shifted_points_refine = self.transformer_refine(
                    ref_poitns,
                    memory,
                    mlvl_feats,
                    mlvl_masks,
                    query_embeds,
                    mlvl_positional_encodings,
                    reg_branches=self.refine_kpt_branches if self.with_box_refine else None,  # noqa:E501
                    cls_branches=self.refine_cls_branches if self.as_two_stage else None,  # noqa:E501
                    num_gts=num_gts
            )
        hs = hs.permute(0, 2, 1, 3)
        outputs_classes = []
        ...
        for lvl in range(hs.shape[0]):
            ...
            tmp = self.refine_kpt_branches[lvl](hs[lvl])
            if reference.shape[-1] == 4:
                tmp += reference
            else:
                assert reference.shape[-1] == 2
        ...
        # 计算loss
        target_labels = target_labels.unsqueeze(-1).expand(-1, self.num_keypoints) 
        # target_points    -->   num_gt, num_keypoints, 2
        # target_labels    -->   num_gt, num_keypoints
        # outputs_classes  -->   num_stage, num_gt, num_keypoints, num_classes
        # outputs_coords   -->   num_stage, num_gt, num_keypoints, 2
        
        # 每个stage for 一个循环
        for i, (cls_scores, pred_points) in enumerate(zip(outputs_classes, outputs_coords)):
            
            imgs_wh_ = torch.cat([imgs_whwh[j].unsqueeze(0).expand(num_gts[j], -1, -1)
                       for j in range(len(num_gts))], dim=0)
            # kpt cls
            loss_cls = self.loss_cls(cls_scores.reshape(-1, self.num_classes),
                                     target_labels.reshape(-1),
                                     reduction_override='none').sum() / max(1, sum(num_gts) * num_points)
            ...
            for i_pos, (row_inds, col_inds, preds_, targets_) in \
                enumerate(zip(target_vis_row_inds, target_vis_col_inds, pred_points, target_points)):
            ...
            loss_kpt = self.loss_bbox(vis_preds, vis_targets,
                                      reduction_override='none').sum() / max(1, vis_targets.size(0))
            losses[f'd{i}.loss_point_refine'] = loss_kpt * 0
...
#             select_coords = torch.cat([select_coords, (gt_points.unsqueeze(1).flip(-1) / 16).long()], dim=1)
    ...
    def get_semantic_centers(self, 
        ...
        return sampled_points, point_feats, visible_weights, semantic_points
