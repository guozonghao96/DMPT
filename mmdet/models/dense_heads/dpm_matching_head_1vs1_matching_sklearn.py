...
#                                                                     vit_feat[i_img],
        ...
        for i_img in range(num_imgs):
            ...
            with torch.no_grad():                
                ...
                # num_gt, num_classifier, num_parts
                part_labels = labels.reshape(-1, 1, 1).repeat(1, self.num_classifier, self.num_semantic_points)
                # num_gt, num_classifier, num_parts
                ...
                # 把invisible的loss变成0
                part_matching_losses = part_matching_losses * part_visibles.reshape(num_gt, 1, self.num_semantic_points)
                matched_row_inds = []
                matched_col_inds = []
                for i_gt_, matching_losses in enumerate(part_matching_losses.cpu().numpy()):
                    matched_row_inds_, matched_col_inds_ = linear_sum_assignment(matching_losses)
                    matched_row_inds_ = torch.as_tensor(matched_row_inds_).to(part_matching_losses.device)
                    matched_col_inds_ = torch.as_tensor(matched_col_inds_).to(part_matching_losses.device)
