...
#                                                                     vit_feat[i_img],
        ...
        for i_img in range(num_imgs):
            ...
            with torch.no_grad():
                ...
                # 先softmax后mean(sum)
                # 有些是gt的part是invisible的，本质上在除的时候应该是按照gt的part的invisible的数量来出（算loss的时候一定要这样），但是由于gt内部的匹配，实际上并不影响匹配结果，可以简单写
                # 注意0，不能softmax，这种0进行softmax是有贡献的，除非后面乘0,这个必须要处理
                ...
                part_matching_scores = (part_matching_scores.softmax(-1) * part_visibles).sum(-2) \
                            + add_instance_scores.softmax(-1) # num_gt, 1, num_classes
                
                # num_gt, num_matchings ^ 2, num_classes / num_gt, num_matchings ^ 2, 1
                part_matching_scores = part_matching_scores / (part_visibles.sum(-2) + 1)
                part_matching_scores = part_matching_scores.reshape(-1, self.num_classes)
            ...
            losses_dpm['dpm_bce_loss'] += self.bce_loss(object_score, labels).mean() + instance_part_score.sum() * 0
