...
class DPMHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.part_filters = nn.ModuleList(
            nn.Linear(out_channels, num_classes) for _ in range(num_semantic_points))
        if not self.semantic_deform:
            self.part_deform = nn.Parameter(
                torch.tensor([0., 0., 1., 1.]).reshape(1, -1).repeat(num_semantic_points, 1), 
                requires_grad=True)
        else:
            self.part_deform = nn.Parameter(
                torch.tensor([0., 0., 1., 1.]).reshape(1, 1, -1).repeat(num_semantic_points,
                                                                        num_classes, 1), 
                requires_grad=True)
...
#                                                                     vit_feat[i_img],
        ...
        root_scores, part_scores = self(vit_feat_dpm) # bs, N, 20 # bs, num_parts, N, 20
        # calculate matched feature
        for i_img in range(num_imgs):
            labels = gt_labels[i_img]
            visible_weights = all_visible_weights[i_img]
            semantic_points = all_semantic_points[i_img] / 16
            part_based_masks = all_part_based_masks[i_img].long()
            root_mask = F.interpolate(fg_masks[i_img].unsqueeze(1), 
                                      (featmap_sizes[2][0], featmap_sizes[2][1]),
                                      mode='bilinear').squeeze(1).long()
            ...
            num_gt = len(labels)
#             instance_root_score = torch.gather(root_score.permute(2, 0, 1), # num_classes, h, w
#                                              index=labels.reshape(-1, 1, 1).repeat(1, *featmap_sizes[2]),
#                                              dim=0) # num_gt, h, w 
#             instance_part_score = torch.gather(part_score.permute(3, 0, 1, 2), # num_classes, num_part, h, w
#                                              index=labels.reshape(-1, 1, 1, 1).repeat(1, 
#                                                                                       self.num_semantic_points, 
#                                                                                       *featmap_sizes[2]), dim=0) # num_gt, num_part, h, w
            instance_root_score = root_score.unsqueeze(0).repeat(num_gt, 1, 1, 1) # num_gt, h, w, num_classes
            instance_part_score = part_score.unsqueeze(0).repeat(num_gt, 1, 1, 1, 1) # num_gt, num_part, h, w, num_classes
            
            # part offset
            if self.semantic_deform:
                part_deform = self.part_deform.reshape(1, self.num_semantic_points, 
                                        self.num_classes, 1, 4).repeat(num_gt, 1, 1, 1, 1)
                part_deform = torch.gather(part_deform, 
                                          index=labels.reshape(-1, 1, 1, 1, 1).repeat(1, self.num_semantic_points, 1, 1, 4),
                                          dim=2) # num_gt, num_part, 1, 1, 4
            else:
                part_deform = self.part_deform.reshape(1, self.num_semantic_points, 1, 1, 4)
                
            xx, yy = torch.meshgrid(torch.arange(featmap_sizes[2][1]).to(semantic_points.device), 
                                    torch.arange(featmap_sizes[2][0]).to(semantic_points.device))
            xx_norm = xx / (torch.ones_like(xx) * featmap_sizes[2][1])
            yy_norm = yy / (torch.ones_like(yy) * featmap_sizes[2][0])
            all_grids.append(torch.stack([xx_norm.reshape(-1),
                                                 yy_norm.reshape(-1)], dim=-1))
            norm_semantic_points = semantic_points / \
                    torch.as_tensor(featmap_sizes[2]).to(semantic_points.device).flip(0).reshape(1, 1, 2) # num_gt, num_parts, 2
            ...
            deform_cost = torch.cat([deform_cost_term1, 
                                     deform_cost_term2], dim=-1).reshape(-1, 
                                                                         ...
                                                                         featmap_sizes[2][0],
                                                                         4).permute(0, 1, 3, 2, 4)
            # num_gt, num_part, h, w, 4
            weighted_deform_cost = (deform_cost * part_deform).sum(-1)
            
            # filter score
            all_root_scores.append(instance_root_score)
            all_part_scores.append(instance_part_score)
            # all_root_scores.append(instance_root_score * root_mask.unsqueeze(-1))
            # all_part_scores.append(instance_part_score * part_based_masks.unsqueeze(-1))
            all_deformable_costs.append(deform_cost * part_deform * part_based_masks.unsqueeze(-1))
            all_weighted_deform_costs.append(deform_cost * part_deform)
        
            masked_instance_root_score = (instance_root_score * root_mask.unsqueeze(-1)).flatten(1, 2).sum(1).unsqueeze(1) / (root_mask.flatten(1).sum(1).unsqueeze(1).unsqueeze(-1) + 1)
            # deformable part
            masked_instance_part_score = (instance_part_score - self.deformable_cost_weight * weighted_deform_cost.unsqueeze(-1)) * part_based_masks.unsqueeze(-1)
            masked_instance_part_score = masked_instance_part_score.flatten(2, 3).sum(2) / (part_based_masks.flatten(2).sum(2).unsqueeze(-1) + 1)
            # losses
            object_score = torch.cat([masked_instance_root_score, masked_instance_part_score], dim=1).sum(1)
            
            losses_dpm['dpm_bce_loss'] += self.bce_loss(object_score.softmax(-1), labels).mean()
            
            # part-based model select part points
            masked_part_proposals_map = (instance_part_score - self.deformable_cost_weight * weighted_deform_cost.unsqueeze(-1)) * part_based_masks.unsqueeze(-1)
            masked_part_proposals_map = torch.gather(
                masked_part_proposals_map,
                index=labels.reshape(-1, 1, 1, 1, 1).repeat(1, self.num_semantic_points, featmap_sizes[2][0], featmap_sizes[2][1], 1),
                dim=-1)[..., 0] # num_gt, num_part, h, w
            masked_part_proposals_map = F.interpolate(
                masked_part_proposals_map, size=(featmap_sizes[2][0] * 16, featmap_sizes[2][1] * 16), # num_gt, num_part, h, w
                mode='bilinear') # 忘记sigmoid了，不过都是线性变化，应该问题不大 
            # 问题大，因为logits可能存在着负数，你不softmax，他有可能是负数呢
            # 并且还得在没乘part mask之前加
            ...
            y_coords = max_flatten_index // (featmap_sizes[2][1] * 16) + 0.5
            x_coords = max_flatten_index % (featmap_sizes[2][1] * 16) + 0.5
            part_locations = torch.stack([x_coords, y_coords], dim=-1)
            visible_weights = (part_based_masks.flatten(-2).sum(-1) > 0)
        ...
        attnshift_results['all_part_scores'] = all_part_scores
        attnshift_results['all_deformable_costs'] = all_deformable_costs
        attnshift_results['all_weighted_deform_costs'] = all_weighted_deform_costs
...
def get_center_coord(maps, rois, obj_label, num_max_keep=50, num_max_obj=3):
    ...
    for i_obj, map_ in enumerate(maps):
        ...
        for i_prot in range(map_.shape[0]):
            ...
            part_mask = map_[(coord_top2[:, 0]==map_area_idxsort[i_prot])[:map_.shape[0]]].mean(dim=0) > 0.9
