...
class ScaleStageCascadeRPNHead(RPNHead):
...
#         self.num_classes = num_classes
        ...
        super(ScaleStageCascadeRPNHead, self).__init__(
            in_channels,
...
#                 self.init_cfg['override'].append(dict(name='rpn_cls'))
            ...
            for i in range(self.stacked_convs):
                ...
                self.reg_convs.append(
                    ConvModule(
                        ...
                        conv_cfg=None,
                        norm_cfg=None))
            self.score0 = nn.Conv2d(self.feat_channels, 
                                   self.num_base_priors * 20, #self.num_classes,
                                   1)
            self.score1 = nn.Conv2d(self.in_channels, 
                                   self.num_base_priors * 20, #self.num_classes,
                                   1)
            self.single_reg = nn.Conv2d(self.feat_channels, self.num_anchors * 4, 1)
        else:
            self.rpn_conv_cls = AdaptiveConv(self.in_channels, self.feat_channels,
                                         **self.adapt_cfg)
            self.rpn_conv_reg = AdaptiveConv(self.in_channels, self.feat_channels,
                                         **self.adapt_cfg)
            if self.with_cls:
    ...
    def forward_single(self, x, offset):
        ...
        if self.with_mil:
            ...
            bridged_x = [cls_feat, reg_feat]
            bbox_pred = self.single_reg(reg_feat)
            scores0 = self.score0(cls_feat)
            scores1 = self.score1(cls_feat)
            return bridged_x, None, bbox_pred, scores0, scores1
        else:
            bridged_x = x
            x_cls = self.relu(self.rpn_conv_cls(x[0], offset))
            x_reg = self.relu(self.rpn_conv_reg(x[1], offset))
            if self.bridged_feature:
                bridged_x = x  # update feature
            cls_score = self.rpn_cls(x_cls) if self.with_cls else None
            bbox_pred = self.rpn_reg(x_reg)
    ...
    def get_targets(self,
        ...
        if isinstance(self.assigner, RegionAssigner):            
            ...
            cls_reg_targets = super(ScaleStageCascadeRPNHead, self).get_targets(
                anchor_list,
                valid_flag_list,
                gt_bboxes,
                img_metas,
    ...
    def loss_(self,
        ...
        scores0 = torch.cat(scores0, dim=1)
        scores1 = torch.cat(scores1, dim=1)
        ...
        for _, (anchors_, gt_labels_, gt_bboxes_, cls_scores0_, cls_scores1_,
                bbox_preds_) in enumerate(
                    zip(anchors, gt_labels, gt_bboxes, scores0, scores1, bbox_preds)):
            ...
            num_scale = 4
            matched_cls_scores0 = cls_scores0_[matched].reshape(-1, num_scale, self.pre_anchor_topk, 20) # 分数
            matched_cls_scores1 = cls_scores1_[matched].reshape(-1, num_scale, self.pre_anchor_topk, 20) # propsal
            num_gt = matched_cls_scores0.size(0)
            matched_cls_scores0 = matched_cls_scores0.mean(-2).softmax(-1)
            matched_cls_scores1 = matched_cls_scores1.mean(-2).softmax(-2)
            
            bag_scores = matched_cls_scores0 * matched_cls_scores1 # 分数维度 * proposal维度
            # 计算loss
#             labels_ = torch.zeros(num_gt, dtype=torch.long).to(bag_scores.device)
            anchor_mil_loss += self.loss_mil(bag_scores.sum(1), gt_labels_[0::num_scale])
            
            with torch.no_grad():
                topk_merge = 1
                pseudo_proposals = gt_bboxes_.reshape(num_gt, num_scale, 4)
                ...
                temp_bag_scores = torch.gather(bag_scores, dim=-1, index=index)[..., 0]
#                 temp_bag_scores = bag_scores[..., 0]
                _, pseudo_index = temp_bag_scores.topk(topk_merge, dim=-1)
            ...
            index_ = index.reshape(num_gt, num_scale, 1, 1).repeat(1, 1, self.pre_anchor_topk, 1)
            anchor_reg_mil_weight = bag_scores[..., 0].reshape(num_gt, num_scale, 1, 1).repeat(1, 1, self.pre_anchor_topk, 1) * \
                                        torch.gather(cls_scores0_[matched].reshape(-1, num_scale, self.pre_anchor_topk, 20).softmax(-1), dim=-1, index=index_)[..., 0].unsqueeze(-1)
            
            anchor_reg_loss += self.loss_bbox(
                bbox_preds_[matched],
                matched_object_targets,
                weight=anchor_reg_mil_weight.reshape(-1, self.pre_anchor_topk, 1).detach(),
                reduction_override='none').sum() / max(1, num_gt)
        
        return dict(anchor_mil_loss=anchor_mil_loss,
                    anchor_reg_loss=anchor_reg_loss), pseudo_gt_bboxes
...
@HEADS.register_module()
class ScaleCascadeRPNHead(BaseDenseHead):
    ...
    def __init__(self, num_stages, stages, train_cfg, test_cfg, init_cfg=None):
        super(ScaleCascadeRPNHead, self).__init__(init_cfg)
...
#         # 其label为 (num_gt * num_scale)
        gt_labels = [labels.reshape(-1, 1).repeat(1, num_scale) for labels in gt_labels]
        overall_gt_labels = []
        overall_gt_bboxes = []
...
#         gt_bboxes = overall_gt_bboxes
        ...
        for i in range(self.num_stages):
            ...
            if i == 0:
                x, cls_score, bbox_pred, scores0, scores1 = stage(x, offset_list)
                rpn_loss_inputs = (anchor_list, valid_flag_list, cls_score, 
                                   bbox_pred, scores0, scores1, gt_labels, gt_bboxes, img_metas)
                stage_loss, gt_bboxes = stage.loss_(*rpn_loss_inputs)
            else:
        ...
        if proposal_cfg is None:
            ...
            proposal_list = self.stages[-1].get_bboxes(anchor_list, cls_score,
                                                       bbox_pred, img_metas,
                                                       self.test_cfg)
            return losses, proposal_list, gt_bboxes
    ...
    def simple_test_rpn(self, x, img_metas):
        ...
        for i in range(self.num_stages):
            ...
            if stage.adapt_cfg['type'] == 'offset':
                ...
                offset_list = None
            if i == 0:
                x, cls_score, bbox_pred, _, _ = stage(x, offset_list)
            else:
                x, cls_score, bbox_pred = stage(x, offset_list)
            if i < self.num_stages - 1:
                anchor_list = stage.refine_bboxes(anchor_list, bbox_pred,
