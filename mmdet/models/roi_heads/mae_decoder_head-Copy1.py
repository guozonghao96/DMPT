...
class MAEDecoderHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def _mask_forward_train(self, x, sampling_results, bbox_feats, gt_masks,
        ...
        # 因为semantic的第一个点是gt，所以需要去掉
        num_semantics = semantic_points[0].size(1)
        pseudo_points = [
            torch.cat([semantic_points[i][:, 1:], pseudo_points[i]], dim=1)
                for i in range(num_imgs)]
        pseudo_bin_labels = [
            torch.cat([torch.ones_like(semantic_points[i][:, 1:][..., 0]).long(), 
        ...
        # semantic 中 unvisible的点的坐标为-10000，不会在目标bbox内
        point_ignores = (
            (new_sites[:, :, 0] < 0)
            | (new_sites[:, :, 0] > 1)
            | (new_sites[:, :, 1] < 0)
