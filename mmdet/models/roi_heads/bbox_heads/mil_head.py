...
#             self.fc2_e = nn.Linear(hidden_channels, hidden_channels)
    ...
    def forward(self, x, x_edge=None, num_scale=None, loc_scores=None):
        if self.pooling_type == 'attn':
            ...
            x = F.relu(self.fc2(x), inplace=True)
            # dual-stream 
            score1 = self.score1(x).softmax(-1) # cls branch # num_gt, 7, 20
            score2 = self.score2(x).softmax(-2) # scale branch
...
#         losses['loss_mil'] = self.loss_mil(bag_score.sum(-2), gt_labels).sum() / max(1, num_gt)
        losses['loss_mil'] = self.loss_mil(bag_score.sum(-2), gt_labels).mean()
        losses['mil_pos_acc'] = accuracy(bag_score.sum(-2), gt_labels)
        return losses
