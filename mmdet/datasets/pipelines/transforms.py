...
class RandomCrop:
    ...
    def __init__(self,
        ...
        self.bbox2point = {
            'gt_bboxes': 'gt_points',
            'gt_bboxes_ignore': 'gt_points_ignore'
        }
    ...
    def _crop_data(self, results, crop_size, allow_negative_crop):
        ...
        for key in results.get('bbox_fields', []):
            ...
            # point fields e.g. gt_points and gt_points_ignore
            point_key = self.bbox2point.get(key)
            if point_key in results:
                point_offset = np.array([offset_w, offset_h], dtype=np.float32)
                points = results[point_key][valid_inds] - point_offset
                if self.bbox_clip_border:
                    points[:, 0] = np.clip(points[:, 0], 0, img_shape[1])
                    points[:, 1] = np.clip(points[:, 1], 0, img_shape[0])
                results[point_key] = points
        # crop semantic seg
        for key in results.get('seg_fields', []):
