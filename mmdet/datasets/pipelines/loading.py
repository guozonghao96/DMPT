...
class LoadAnnotations:
    ...
    def _load_points(self, results):
        ...
        if self.denorm_bbox:
            point_num = results['gt_points'].shape[0]
            if point_num != 0:
                h, w = results['img_shape'][:2]
                results['gt_points'][:, 0:1] *= w
                results['gt_points'][:, 1:2] *= h

        gt_points_ignore = ann_info.get('points_ignore', None)
        if gt_points_ignore is not None:
            results['gt_points_ignore'] = gt_points_ignore.copy()
            results['point_fields'].append('gt_points_ignore')
